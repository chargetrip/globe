...
export default function getImageAlphaSample(
  ...
  if (!imageData) {
    throw Error('Globe - Could not load alpha sample image')
  }
  ...
  const tx = Math.min((emod(u, 1) * imageData.width) | 0, imageData.width - 1)

  const ty = Math.min((emod(v, 1) * imageData.height) | 0, imageData.height - 1)
